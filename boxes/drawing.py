...
def normalize_arc_angles(start_angle: float, end_angle: float, orientation: int) -> tuple[float, float]:
    """
    Ensure the provided arc angles describe a continuous sweep in the requested orientation.

    Positive orientation (CCW) expects the end angle to be larger than the start angle, while
    negative orientation (CW) expects the opposite. The function shifts the end angle by whole
    turns until the relationship matches the orientation, keeping the start angle unchanged.
    The EPS tolerance avoids unnecessary adjustments when the sweep is already consistent.
    
    This normalization is necessary so later steps that approximate arcs or compute extents
    do not receive inverted sweeps, which would otherwise break path expansion and layout.
    """
    if orientation > 0:
...
def arc_to_cubic_segments(cx: float, cy: float, radius: float, start_angle: float, end_angle: float) -> list[list[float]]:
    """
    Convert an arc into cubic Bezier curve segments that approximate the same geometry.

    The routine splits the sweep into at most quarter-circle spans because those can be
    represented accurately with cubic Beziers. Each segment is emitted as a path command
    following the SVG 'C' format: ['C', end_x, end_y, ctrl1_x, ctrl1_y, ctrl2_x, ctrl2_y].
    Zero-length sweeps are ignored to prevent redundant commands.
    
    The renderer only understands lines and cubics, so this conversion is required to keep
    arc geometry visible in backends that lack native arc support.
    """
    delta = end_angle - start_angle
    if abs(delta) < 1e-12:
        return []
    segments = max(1, math.ceil(abs(delta) / (math.pi / 2.0)))
...
def angle_on_arc(angle: float, start: float, end: float, orientation: int) -> float | None:
    """
    Map a raw angle onto the normalized arc span when it falls inside the sweep.

    Because arcs may cover multiple turns, the function wraps the candidate angle forward
    (CCW) or backward (CW) in multiples of tau (2π) until it lies within the inclusive bounds.
    If the wrapped angle is outside the sweep, None is returned.
    EPS ensures angles that land exactly on the boundaries are considered in range.
    
    Bounding-box computations rely on these wrapped angles; without this helper we would miss
    extremal points of arcs and end up with incorrect extents and clipping.
    """
    full_turn = TAU
    if orientation > 0:
...
def expand_path_arcs(commands):
    """
    Replace arc commands in a path with cubic Bezier segments while leaving other commands untouched.

    Each arc ('A') command is expanded using arc_to_cubic_segments, preserving the sequence of
    drawing commands so that downstream consumers that only understand Beziers receive an equivalent path.
    
    The rest of the pipeline assumes paths are expressed with primitives it can rasterize,
    so expanding arcs here is mandatory to prevent missing geometry during rendering/export.
    """
    expanded = []
    for cmd in commands:
        letter = cmd[0]
        if letter == 'A':
            _, _, _, cx, cy, radius, start_angle, end_angle, _ = cmd
...
class Surface:
    ...
    def prepare_paths(self, inner_corners, dogbone_radius=None):
        """
        Prepare every stored path before rendering by applying corner treatments and normalizing coordinates.

        Each path receives the `Path.faster_edges` post-processing configured via `inner_corners`, optionally
        adding dogbone reliefs when a radius is provided. Once paths are updated, `_adjust_coordinates` recenters
        and scales the geometry and returns the resulting output extents.
        
        This preprocessing step is required so the generated toolpaths match the chosen corner strategy and
        so the final drawing is positioned correctly for the downstream renderers and exporters.
        """
