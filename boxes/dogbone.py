...
def _point_on_arc(segment: ArcSegment, point: Point, eps: float) -> bool:
...
def _trim_overlaps(path: PathLike, eps: float) -> None:
    ...
    def consider_right_arc(line_data: LineState | None, right_arc: ArcSegment) -> None:
        if line_data is None or not line_data.left_arcs:
            return
        update_best(line_data, evaluate_candidate(line_data, right_arc))

    def evaluate_candidate(line_data: LineState, right_arc: ArcSegment) -> Candidate | None:
        best: Candidate | None = None
    ...
    def record_overlap(candidate: Candidate) -> None:
    ...
    def finalize_line_state(close_subpath: bool = False) -> None:
        ...
        if line_state:
            best_candidate = line_state.best
            if best_candidate:
        ...
        if close_subpath and first_line_state:
            best_candidate = first_line_state.best
            if best_candidate:
                record_overlap(best_candidate)
        if close_subpath:
    ...
    for index, segment in enumerate(path):
        ...
        if code == "M":
            ...
            line_state = LineState(index=index, start=start, end=end, left_arcs=arcs_since_line)
            arcs_since_line = []
            current = end
